...
public interface ContextFactory<C> {
	...
	default @NotNull Context<C> createContext(@NotNull CommandSource<C> commandSource,
		...
		return createContext(commandSource, command, queue, CommandFlagExtractorImpl::createNative);
	}
	 ...
	 */
	ResolvedContext<C> createResolvedContext(@NotNull Command<C> command,
