...
public final class CommandTree<C> {
	...
	CommandTree(Command<C> command) {
		this.root = new CommandNode<>(command);
		//parse(command);
	}
	
	public static <C> CommandTree<C> create(Command<C> command) {
		return new CommandTree<>(command);
	}
	
	public static <C> CommandTree<C> parsed(Command<C> command) {
		CommandTree<C> tree = create(command);
		tree.parseCommandUsages();
		return tree;
	}
	
	public void parseCommandUsages() {
		for(CommandUsage<C> usage : root.data.getUsages()) {
			parseUsage(usage);
		}
	}
	
	public void parseUsage(CommandUsage<C> usage) {
